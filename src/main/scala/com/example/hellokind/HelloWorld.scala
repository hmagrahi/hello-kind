...
object HelloWorld {
  ...
  def impl[F[_]: Applicative](databaseConfig: DatabaseConfig): HelloWorld[F] = new HelloWorld[F] {
    def hello(n: HelloWorld.Name): F[HelloWorld.Greeting] =
      Greeting("Hello, " + n.name + s", with data base information $databaseConfig! Don't do this").pure[F]
  }
}
